...
def define_G(input_nc, output_nc, ngf, netG, norm='batch', use_dropout=False, init_type='normal', init_gain=0.02, gpu_ids=[], feat_dim=512):
    ...
    elif netG == 'unetac_256':
        net = UnetACGenerator(input_nc, output_nc, 8, ngf, norm_layer=norm_layer, use_dropout=use_dropout)
    elif netG == 'unetac_adain_256':
        net = unet_generator_ac_adain(input_nc, output_nc, ngf, feat_dim)
    elif netG == 'unet_adain_256':
        net = unet_generator_adain(input_nc, output_nc, ngf, feat_dim)
    elif netG == 'unetr_256':
        net = UnetGeneratorRefine(input_nc, output_nc, 8, ngf, norm_layer=norm_layer, use_dropout=use_dropout)
    else:
...
class UnetACGenerator(nn.Module):
    """Create a Unet-based generator"""
    def __init__(self, input_nc, output_nc, num_downs, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False):
        """Construct a Unet generator
        Parameters:
        ...
        It is a recursive process.
        """
        super(UnetACGenerator, self).__init__()
        # construct unet structure
        unet_block = UnetSkipConnectionBlock(ngf * 8, ngf * 8, input_nc=None, submodule=None, norm_layer=norm_layer, innermost=True)  # add the innermost layer
        for i in range(num_downs - 5):          # add intermediate layers with ngf * 8 filters
            unet_block = UnetSkipConnectionBlock(ngf * 8, ngf * 8, input_nc=None, submodule=unet_block, norm_layer=norm_layer, use_dropout=use_dropout)
        ...
        unet_block = UnetSkipConnectionBlock(ngf * 2, ngf * 4, input_nc=None, submodule=unet_block, norm_layer=norm_layer)
        unet_block = UnetSkipConnectionBlock(ngf, ngf * 2, input_nc=None, submodule=unet_block, norm_layer=norm_layer)
        self.model = UnetSkipConnectionACBlock(output_nc, ngf, input_nc=input_nc, submodule=unet_block, outermost=True, norm_layer=norm_layer)  # add the outermost layer
...
class unet_generator_adain(nn.Module):
    
    def __init__(self, input_nc, output_nc, ngf, feat_dim = 512):
        super(unet_generator_adain, self).__init__()
...
class UnetGeneratorRefine(nn.Module):
    """Create a Unet-based generator"""
    ...
    def __init__(self, input_nc, output_nc, num_downs, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False):
        ...
        It is a recursive process.
        """
        super(UnetGeneratorRefine, self).__init__()
        # construct unet structure
        unet_block = UnetSkipConnectionRefineBlock(ngf * 8, ngf * 8, input_nc=None, submodule=None, norm_layer=norm_layer, innermost=True)  # add the innermost layer
        for i in range(num_downs - 5):          # add intermediate layers with ngf * 8 filters
            unet_block = UnetSkipConnectionRefineBlock(ngf * 8, ngf * 8, input_nc=None, submodule=unet_block, norm_layer=norm_layer, use_dropout=use_dropout)
        # gradually reduce the number of filters from ngf * 8 to ngf
        unet_block = UnetSkipConnectionRefineBlock(ngf * 4, ngf * 8, input_nc=None, submodule=unet_block, norm_layer=norm_layer)
        unet_block = UnetSkipConnectionRefineAddinputBlock(ngf * 2, ngf * 4, input_nc=None, dadd_nc=input_nc, submodule=unet_block, norm_layer=norm_layer)
        unet_block = UnetSkipConnectionRefineAddinputBlock(ngf, ngf * 2, input_nc=None, dadd_nc=input_nc, submodule=unet_block, norm_layer=norm_layer)
        self.model = UnetSkipConnectionRefineAddinputBlock(output_nc, ngf, input_nc=input_nc, dadd_nc=input_nc, submodule=unet_block, outermost=True, norm_layer=norm_layer)  # add the outermost layer

    def forward(self, input):
        """Standard forward"""
        im = nn.Upsample(scale_factor=[0.5,0.5],mode='bilinear')(input)
        #print im.shape, input.shape
        return self.model(input, im)
